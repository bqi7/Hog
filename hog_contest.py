...
TEAM_NAME = 'THE GODZ' # Change this line!
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while n <= num_rolls:
        result = dice()
        if result != 1 and total != 1:
            n, total = n + 1, total + result
        else:
           n, total = n + 1, 1
    return total
    # END PROBLEM 1
...
def free_bacon(score):
    ...
    return abs(ones_digit - tens_digit) + 2
    # END PROBLEM 2
    ...
    def say(score0, score1):
        if who == 0:
            ...
            if difference > previous_high and difference == 1:
                    print(difference, "point! That's the biggest gain yet for Player 0")
            elif difference > previous_high:
                    print(difference, "points! That's the biggest gain yet for Player 0")
            else:
        ...
        if who == 1:
            ...
            if difference > previous_high and difference == 1:
                print(difference, "point! That's the biggest gain yet for Player 1")
            elif difference > previous_high:
                    print(difference, "points! That's the biggest gain yet for Player 1")
            else:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    roll, max_average = 1, 0
    while roll <= 10:
        average_dice = make_averaged(roll_dice, num_samples)
        average = average_dice(roll, dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 11
    if free_bacon(opponent_score) >= margin or opponent_score % (free_bacon(opponent_score) + score) == 0:
        return 0
    return num_rolls
    # END PROBLEM 11


def final_strategy(score, opponent_score, margin = 9, num_rolls = 4):
    """This strategy uses swap_strategy() as its base with a higher default value
    for margin, 9, and num_rolls, 4. As score gets closer to 100, the default values
    of margin and num_rolls are reduced.
    """
    # BEGIN PROBLEM 12
    if score < 85:
        num_rolls = swap_strategy(score, opponent_score, margin, num_rolls)
    elif score < 90:
        num_rolls = swap_strategy(score, opponent_score, margin = 7, num_rolls = 3)
    else:
        num_rolls = swap_strategy(score, opponent_score, margin = 5, num_rolls = 2)
    return num_rolls
    # END PROBLEM 12
